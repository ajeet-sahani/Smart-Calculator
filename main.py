...
MDScreen:
	MDBoxLayout:
		...
		MDSegmentedControl:
			...
			size_hint_x:1
			md_bg_color:0,0,0.7,1
			on_active:app.segment_switch(*args)
...
class Calculator(MDApp):
	...
	def segment_switch(self, instance, segment_item):
		if segment_item.text == "Calculator":
			self.root.ids.sm.current = "calculator"
		else:
			self.root.ids.sm.current = "converter"
	...
	def calculate(self):
		try:
			...
			result = eval(experssion, {"math":math })
